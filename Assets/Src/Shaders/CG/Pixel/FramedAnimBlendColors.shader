Shader "Venat/Pixel/FramedAnimBlendColors"
{
    // C ---- D
	// |      |
	// |      |
	...
	Properties
	{	
		_ColorFrame("Color Frame", Color) = (1, 0, 0, 1)
		_FrameWidth("Frame Width", Range(0, 100)) = 0

		[Toggle]
		_InvertedFrame("Invert Frame", Float) = 0
	...
	SubShader
	{
		Pass
		{
			...
			{	
				...
				if ((i.uv.x <= fwidth(i.uv.x) * _FrameWidth || i.uv.x >= 1 - fwidth(i.uv.x) * _FrameWidth) || (i.uv.y <= fwidth(i.uv.y) * _FrameWidth || i.uv.y >= 1 - fwidth(i.uv.y) * _FrameWidth))
				{
					return _InvertedFrame ? finalColorRGBA : _ColorFrame;
				}
				else 
				{
					return _InvertedFrame ? _ColorFrame : finalColorRGBA;
				}	
			}
			ENDCG
		}
