Shader "Custom/Mixed/Outline/StaticNoisyOutline"
{
	// This mixed shader is meant to work in a similar way to the "NoisyOutline" shader but applying noise inside
	// an static outline and drawing transparencies through it instead of using the noise for the outlining itself
		...
		Pass
		{
			...
			CGPROGRAM
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					// make a harder edge
					float4 texturedRim = saturate(rim.a * _DissolveAmount); 

					// extra edge, subtracting the textured rim
					float4 extraRim = (saturate((_Edge + rim.a) * _DissolveAmount) - texturedRim);

					// combine both with colors
					half4 result = (_OutlineColor_2 * texturedRim) + (_OutlineColor_1 * extraRim); 
					clip(result.a < _DissolveAmount ? 1 : -1);
					//clip(rim > _DissolveAmount ? 1 : -1);
